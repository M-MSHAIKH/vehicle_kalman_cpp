...
public:
    ...
    EKF(double dt, double l) : dt(dt), l(l) {} // Constructor to initialize wheelbase and time step, useful for initializing the class
    ...
    std::tuple <Eigen::Matrix<double, num_states,1>, Eigen::Matrix<double, num_states, num_states>> predict(const Eigen::Matrix<double, num_states,1>& state, 
                                                                                                            ...
                                                                                                            const Eigen::Matrix<double, num_controls, num_controls>& Q_u) {
        // Implement the prediction step of the EKF
        KinematicBicycle<num_states, num_controls> model(dt, l); // Initialize the kinematic bicycle model with the time step and wheelbase
        auto result = model.state_transition(state, control_input); // Get the predicted state and Jacobian from the kinematic bicycle model
        Eigen::Matrix<double, num_states, 1> predicted_state = std::get<0>(result);
        Eigen::Matrix<double, num_states, 1> jacobian_var = std::get<1>(result);
        ...
        Eigen::Matrix<double, num_states, num_states> F = jacobian_kinematic_bicycle.F_jacobian(jacobian_var); // Calculate the Jacobian with respect to state variables
        Eigen::Matrix<double, num_states, num_controls> L = jacobian_kinematic_bicycle.L_jacobian(jacobian_var); // Calculate the Jacobian with respect to control inputs
    ...
    std::tuple<Eigen::Matrix<double, num_states,1>, Eigen::Matrix<double, num_states, num_states>, Eigen::Matrix<double, num_states, num_measurements>> update (const Eigen::Matrix<double, num_states, 1>& predicted_state, 
        ...
        // Eigen::Matrix<double, num_states, 1> updated_state = (predicted_state) + (K * (y - h));      // use this if you have y and h separately

        Eigen::Matrix<double, num_states, 1> updated_state = predicted_state + (K * y) ;
        ...
        return std::make_tuple(updated_state, P_upd, K); // Return the updated state and covariance as a tuple                                                                                                        
    }
};